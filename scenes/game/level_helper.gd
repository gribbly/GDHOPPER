...
func set_level_dimensions(dimensions: Vector3) -> void:
	...
	half_width = level_dimensions.x / 2.0
	third_width = level_dimensions.x / 3.0
	two_thirds_width = third_width * 2.0
	half_height = level_dimensions.y / 2.0
	x_min = MARGIN_X
	x_max = level_dimensions.x - MARGIN_X
	y_min = MARGIN_Y
	y_max = level_dimensions.y - MARGIN_Y
...
func get_point(x_type: XType, y_type: YType) -> Vector3:
	...
	match x_type:
		...
		_:
			...
			x = RH.get_random_float(0.0 + MARGIN_X, level_dimensions.x - MARGIN_X)
	
	match y_type:
		YType.surface:
			y = level_dimensions.y
		YType.shallow:
		...
		_:
			...
			y = RH.get_random_float(0.0 + MARGIN_Y, level_dimensions.y - MARGIN_Y)
